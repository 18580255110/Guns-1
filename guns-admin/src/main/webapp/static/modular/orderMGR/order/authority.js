 ...
 * 点击添加订单
 */
Authority.openAuthDlg = function () {
    var me = this;
    if (this.check()) {
        var index = layer.open({
            ...
            fix: false, //不固定
            maxmin: true,
            content: Feng.ctxPath + '/order/authority/wizard/' + Authority.seItem.code
        });
        layer.full(index);
        me.layerIndex = index;
    }
...
Authority.search = function () {
    ...
    Authority.queryParams = queryData;
    Authority.table.refresh({query: queryData});
};

Authority.getQueryParams = function(){
    return Authority.queryParams;
};

$(function () {
    var defaultColunms = Authority.initColumn();
    var table = new BSTable(Authority.id, "/class/list", defaultColunms);
    table.setPaginationType("server");
    table.setQueryParamsGetter(Authority.getQueryParams)
    Authority.table = table.init();
});
